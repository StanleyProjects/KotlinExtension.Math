    ...
    fun copyTest() {
        ...
        issues.forEach { (width, height) ->
            ...
            val message = """
                ...
                points: $points
            """.trimIndent()
            assertEquals(actual.width, expected.width, delta, message)
            assertEquals(actual.height, expected.height, delta, message)
            assertEquals(expected, actual, message)
            assertTrue(expected.eq(other = actual, points = points), message)
        }
        issues.forEach { (width, height) ->
            val expected = sizeOf(width = width, height = height)
            val actual = expected.copy(width = 0.0)
            val message = """
                actual: $actual (${actual.toString(24)})
                expected: $expected (${expected.toString(24)})
                delta: $delta (${delta.toString(24)})
                points: $points
            """.trimIndent()
            assertEquals(actual.width, 0.0, delta, message)
            assertEquals(actual.height, expected.height, delta, message)
        }
        issues.forEach { (width, height) ->
            val expected = sizeOf(width = width, height = height)
            val actual = expected.copy(height = 0.0)
            val message = """
                actual: $actual (${actual.toString(24)})
                expected: $expected (${expected.toString(24)})
                delta: $delta (${delta.toString(24)})
            ...
            """.trimIndent()
            assertEquals(actual.width, expected.width, delta, message)
            assertEquals(actual.height, 0.0, delta, message)
        }
    }
}
